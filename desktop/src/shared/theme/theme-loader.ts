...
/**
 * SVS theme name. SVS is the first-party light theme and reuses GitHub
 ...
 * applied by {@link ThemeProvider} toggling a `data-svs-sidebar` attribute
 * on the document root; the CSS lives in `shared/styles/globals/theme.css`.
 */
export const SVS_THEME_NAME = "svs";

/**
 * SVS Dark theme name. The dark-mode counterpart to {@link SVS_THEME_NAME}:
 ...
 * {@link ThemeProvider} toggles the shared `data-svs-sidebar` attribute for
 * this theme too; the `.dark` root class selects the dark gradient values.
 *
 * SVS and SVS Dark are paired in {@link THEME_PAIRS}, so the picker shows a
 * combined "SVS" tile under System mode (follow-OS) plus a single "SVS" tile
 * under Light and an "SVS Dark" tile under Dark.
 */
export const SVS_DARK_THEME_NAME = "svs-dark";

/** The Shiki bundle SVS borrows its base palette from. */
export const SVS_BASE_THEME: SyntaxThemeName = "github-light";

/** The Shiki bundle SVS Dark borrows its base palette from. */
export const SVS_DARK_BASE_THEME: SyntaxThemeName = "github-dark";

/** Map historical Buzz selections into SVS without changing storage keys. */
export function normalizeSvsThemeName(name: string): string {
  if (name === "buzz") return SVS_THEME_NAME;
  if (name === "buzz-dark") return SVS_DARK_THEME_NAME;
  return name;
}
 ...
 *
 * Most themes map to themselves, but the SVS aliases (`svs` / `svs-dark`)
 ...
 * first; passing a raw SVS alias makes Shiki throw and code blocks fall
 * back to unhighlighted plain text.
 */
export function resolveShikiThemeName(name: string): SyntaxThemeName {
  if (name === SVS_THEME_NAME) return SVS_BASE_THEME;
  if (name === SVS_DARK_THEME_NAME) return SVS_DARK_BASE_THEME;
  return name as SyntaxThemeName;
}

// Available themes. "svs" is an SVS-branded theme that reuses the
...
> = {
  // SVS reuses the github-light palette; its gradient is applied separately.
  svs: () => import("shiki/themes/github-light.mjs"),
  // SVS Dark reuses the github-dark palette; dark gradient applied separately.
  "svs-dark": () => import("shiki/themes/github-dark.mjs"),
...
export const THEME_PAIRS: ReadonlyMap<SyntaxThemeName, SyntaxThemeName> =
  new Map([
    // Light → Dark
    // SVS is the first-party pair; keep it first so it leads every category.
    ["svs", "svs-dark"],
